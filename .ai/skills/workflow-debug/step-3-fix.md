...
2. Dispatch-le avec : `archetype: bug-fix`, la cause racine identifiée à
   `step-1-analyze.md`, et la solution choisie par l'utilisateur à
   `step-2-propose.md`. Demande explicitement le plus petit correctif
...
3. Applique la règle `STATUS:` sur le retour. `verification_gate: true` pour
   ...
   `needs_clarification`.
4. Une fois le correctif vérifié, passe à `step-4-verify.md`.
