...
# bugfix — boucle bug-fix (DEBUG à rôles)
...
Verify, validation utilisateur aux étapes 1/3/5), mappé sur les 5 étapes de
`workflows.yaml` → `workflow-debug` (`analyze: researcher` →
`propose: carried_by orchestrator` → `fix: backend-coder|frontend-coder` →
`review: reviewer` → `confirm: carried_by orchestrator`) : les checkpoints
« Propose » et « Confirm » sont portés par l'orchestrateur lui-même (toi),
jamais délégués — `AskUserQuestion` est indisponible aux subagents.
...
2. `steps/step-1-analyze.md` — rôle `researcher` : reproduire + cause probable
3. `steps/step-2-propose.md` — checkpoint utilisateur : choix entre 2-3 solutions
4. `steps/step-3-fix.md` — rôle `backend-coder`/`frontend-coder`
5. `steps/step-4-verify.md` — rôle `reviewer` + vérification multi-couches +
